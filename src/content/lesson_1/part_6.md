...
nearly as much data in registers. Since we can only directly manipulate
data stored in registers, all the operations we can perform with memory revolve around
bussing data between it and registers. As a result, this part will hopefully be 
more straightforward than the previous ones, since loads and stores are 
quite similar to one another.

## Addressing
In order to read/write a given number of bytes from/to memory, you're going to
have to know which addresses to read. The simplest read/write is of a byte (8 bits).
...
If you're curious, these names come from *Gulliver's Travels* where there was an island
